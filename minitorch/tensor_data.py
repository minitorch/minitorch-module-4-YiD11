...
def to_index(ordinal: int, shape: Shape, out_index: OutIndex) -> None:
    ...
    """
    assert len(out_index) == len(shape), "Index must be same size as shape."
    prod = np.prod(shape)
...
def broadcast_index(
    ...
    for i in range(len(shape)):
        if shape[i] == 1:
            ...
            assert big_shape[i + offset] == shape[i], (
                f"Cannot broadcast {big_shape} to {shape} at dimension {i}"
            )
...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    """
    if len(shape1) == 1 and shape1[0] == 1:
        return shape2
    if len(shape2) == 1 and shape2[0] == 1:
        return shape1
    ...
    for i in range(min_len):
        if shape1[i] != shape2[i] and shape1[i] != 1 and shape2[i] != 1:
            raise IndexingError(
                f"Cannot broadcast {shape1} to {shape2} at dimension {i}"
            )
        if shape1[i] == 1:
    ...
    for i in (shape1 if len(shape1) > len(shape2) else shape2)[min_len:]:
        out_shape.append(i)
